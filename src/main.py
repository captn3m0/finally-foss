...
UPCOMING_FILE = Path("_data/upcoming.yml")
...
def next_upcoming_tag(
    ...
    """Return the earliest future-transitioning tag for this source, or None."""
    ranges = source.get("ranges", [])
    earliest_version: Version | None = None
    earliest_tag_name: str = ""
    earliest_license: str = ""
    earliest_original_license: str = ""
    earliest_delay_yrs: int = 0
    earliest_notes: str = ""
    earliest_transition_date: date | None = None
    ...
    for tag_info in tags:
        ...
        if earliest_transition_date is None or transition_date < earliest_transition_date:
            earliest_version = version
            earliest_tag_name = tag_name
            earliest_license = matched["oss_license"]
            earliest_original_license = matched["original_license"]
            earliest_delay_yrs = matched["delay_yrs"]
            earliest_notes = matched.get("notes", "")
            earliest_transition_date = transition_date

    if earliest_version is None:
        return None

    record: dict = {
        "identifiers": make_purls(owner, repo, earliest_tag_name, repo_url),
        "original_license": earliest_original_license,
        "license": earliest_license,
        "delay_yrs": earliest_delay_yrs,
        "release_date": str(earliest_transition_date),
    }
    if source.get("description"):
        record["description"] = source["description"]
    if source.get("website"):
    ...
    if source.get("tags"):
        record["tags"] = source["tags"]
    if earliest_notes:
        record["notes"] = earliest_notes.strip()
    return record
...
def main() -> None:
    ...
    with httpx.Client(headers=headers, timeout=30) as client:
        for source in sources:
            ...
            try:
                ...
                upcoming = next_upcoming_tag(source, owner, repo, repo_url, tags, today)
                if upcoming:
    ...
    all_upcoming.sort(key=lambda r: r["release_date"])
    ...
    UPCOMING_FILE.write_text(
        "# Auto-generated by src/main.py — do not edit by hand\n"
        + yaml.dump(all_upcoming, default_flow_style=False, sort_keys=False, allow_unicode=True)
    )

    print(f"\nWrote {len(all_releases)} transitioned releases to {RELEASES_FILE}")
    print(f"Wrote {len(all_upcoming)} upcoming releases to {UPCOMING_FILE}")
